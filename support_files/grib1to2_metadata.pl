...
# 6/22/2017: added ECMWF: R, W, MSL, SP, TCC
# 11/21/2017: added support for minutes time units
# 12/2017: added ECMWF RSN
# 4/2018: added more ECMWF, need way to scale and offset data,
#         removed TP as it has a different scale than APCP
#         comverted IMGD (NCEP) -> missing data for other centers than NCEP
#         added suppoort ECMWF ensembles
#         requires wgrib2 v2.0.7+
# 6/2018: added ECMWF O3, SKT, CC -> TCDC, 
#         added scaling (needs new wgrib2) Z->HGT
#         change output from "N hour ave anl,missing=0 to "N hour ave(anl),missing=0"
#           -set_ftime v2 doesn't understand old format
#         old: W -> VVELsfc new: W->VVEL
#
#  BIG PROBLEM CLWMR:  by WMO definition, this is the cloud mixing ratio
#                           a cloud consists of both ice and water
#                      but CLWMR is short for Cloud Water Mixing Ratio and
#                         most people of ice versus water versus vapor.
#                         and CLWMR is is the liquid water mixing ratio.`jjj
#                      in wgrib2 CLWMR is replace by CLMR (v2.0.8)
# 12/2018:        CLWMR (grib1) -> CLLMR (grib2)
# 2/2019:      remove 2nd $center
# 12/2019:   old: CLWMR -> CLLMR  new: CLWMR->CLMR (wgrib2 v2.0.9)
#  3/2020:   PSPCP (pseudo-precip) -> IMGD    Lee Byerle
#  8/2021:   CLWMR  -> CLLMR
# 
$metadata_version = "2021.08.15 NCEP/CPC";
...
#       used in case there needs to be a scaling of variables
        $scaling='';
...
#       scan for ECMWF ens info
# print ">>>>pre B $line_simple\n";
	if ($center == 98) {
            if ($line_simple =~ s/:type=Control forecast 0:/:/) { $ens="ENS=hi-res ctl"; }
            if ($line_simple =~ s/:type=Perturbed forecast ([0-9]*):/:/)  { $ens="ENS=+$1"; }
        }
...
# for forecasts
	if ($conv eq 'fcst') {
	$ftime =~ s/min ave$/ min ave fcst/;
	$ftime =~ s/min acc$/ min acc fcst/;
	...
	$ftime =~ s/^valid (\d*)-(\d*)min$/$1-$2 min ave fcst/;
...
# for analysis
	...
	$ftime =~ s/min ave$/ min ave anl/;
	$ftime =~ s/min acc$/ min acc anl/;
	...
	$ftime =~ s/^valid (\d*)-(\d*)min$/$1-$2 min ave anl/;
...
# 8/2021       elsif ($var eq 'CLWMR') { $var = 'CLMR'; }
        elsif ($var eq 'CLWMR') { $var = 'CLLMR'; }
        elsif ($var eq 'MAXVIG') {
           $var='MAXVIGGG';
        ...
        elsif ($var eq 'PSPCP') { $var='IMGD'; $undef++; }
...
# note ECMWF variables may need some conversion factors
	...
	elsif ($var eq 'MSL') { $var='PRES' ; $lev='mean sea level'; }
	elsif ($var eq 'SP') { $var='PRES' ; $lev='surface'; }
	elsif ($var eq 'W') { $var='VVEL' ; }
	elsif ($var eq 'TCC') { $var='FRACCC' ; }
	elsif ($var eq 'UST') { $var='USSD' ; }
	elsif ($var eq 'VST') { $var='VSSD' ; }
	elsif ($var eq 'SSTK') { $var='WTMP' ; $lev='surface'; }
	elsif ($var eq 'BLH') { $var='HPBL' ; $lev='surface'; }
	elsif ($var eq 'CI') { $var='ICEC' ; $lev='surface'; }
	elsif ($var eq 'SF') { $var='TSNOWP' ; $lev='surface'; }
	elsif ($var eq 'TCW') { $var='TCWAT' ; $lev='surface'; }
	elsif ($var eq '10FG6') { $var='GUST' ; $lev='10 m above ground'; $ftime =~ s/:([0-9-]*)hr/$1 hour max fcst/; }
#       not very good but ECMWF files are not great
	elsif ($var eq 'MN2T6') { $var='TMP'; $lev='2 m above ground'; $ftime =~ s/:([0-9-]*)hr/$1 hour min fcst/; }
	elsif ($var eq 'MX2T6') { $var='TMP'; $lev='2 m above ground'; $ftime =~ s/:([0-9-]*)hr/$1 hour max fcst/; }
	elsif ($var eq 'MN2T') { $var='TMP'; $lev='2 m above ground'; $ftime =~ s/:([0-9-]*)hr/$1 hour min fcst/; }
	elsif ($var eq 'MX2T') { $var='TMP'; $lev='2 m above ground'; $ftime =~ s/:([0-9-]*)hr/$1 hour max fcst/; }
...
#  	elsif ($var eq 'Z') { $var='GP'; }
	...
	elsif ($var eq 'O3') { $var='O3MR'; }
	elsif ($var eq 'CC') { $var='TCDC'; }
	...
	elsif ($var eq 'RSN') { $var='SDEN'; $lev='surface'; }
	elsif ($var eq 'LSM') { $var='LAND'; $lev='surface'; }
	elsif ($var eq 'SKT') { $var='TMP'; $lev='surface'; }
          
#       Z->HGT / 9.80665 `
	elsif ($var eq 'Z') { $var='HGT'; $scaling='data_*=0.101971621' ; }
#   need to scale variables
#       SD = snow depth in m of H2O .. like WEASD which is in mm
	elsif ($var eq 'SD') { $var='IMGD'; $undef++; }
	elsif ($var eq 'CP') { $var='IMGD'; $undef++; }
	elsif ($var eq 'TP') { $var='IMGD'; $undef++; }
#	elsif ($var eq 'TP') { $var='APCP'; }
	elsif ($var eq 'SRO') { $var='IMGD'; $undef++; }
	elsif ($var eq 'TCO3') { $var='IMGD'; $undef++; }

        $n=1;
        $byte=0;

	if ($ens ne '') { $ens=":$ens:";  }
        if ($var eq 'IMGD' && $center != 7) {
           $var="var255_255_1_255_255_255";
        }
        if ( $scaling ne '' ) { $scaling= ':' . $scaling . ':' ; }
        print META "$n:$byte:$date:$var:$lev:$ftime:scale=$dec_scale,$bin_scale:center=$center:subcenter=$subcenter:analysis_or_forecast_process_id=$process:table_3.2=$radius$ens:$scaling\n";
	$n++;
        $line="";
    }
}
