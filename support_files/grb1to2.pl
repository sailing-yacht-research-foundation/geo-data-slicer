...
# v1.6 1/2020 better precision for Gaussian grid
# v1.7 1/2021 use NCEP variable names, needed for wgrib2 v3.0.1
#
...
# $version="v1.5.1  9/2016 W. Ebisuzaki CPC/NCEP";
# $version="v1.5.2  10/2018 W. Ebisuzaki CPC/NCEP";
# $version="v1.6  1/2020 W. Ebisuzaki CPC/NCEP";
$version="v1.7  1/2021 W. Ebisuzaki CPC/NCEP";
...
# wgrib2 v3.0.1 adds ECMWF names, need to use NCEP names

$config=`$wgrib2 -config`;
if ( $config =~ 'default WMO names:' ) { $wgrib2 = "$wgrib2 -names ncep"; }
...
#     give more precision to dlon than grib1 allows
      if (abs($nx*$dx - 360.0) < 0.1) {
         $dx = 360.0 / $nx;
      }

#     give better estimate of lat0 than grib1 allows
      $eps = 3e-14;
      $i = 1;
      ...
      $x = -atan2($z,sqrt(1-$z*$z))*180/3.141592654;

      if (abs($x-$lat0) < 0.01) {  $lat0 = $x; }
      $x=-$x;
      if (abs($x-$lat0) < 0.01) {  $lat0 = $x; }
...
if ($ncep_uv == 1) { 
      mkfifo("$out.uv",0600) or die "mkfifo failed - directory write protected?";
}

if ($packing eq 'c0') {

   if ($fast == 0) {
      mkfifo("$out.c11",0600) or die "mkfifo failed - directory write protected?";
      ...
      mkfifo("$out.1.c1",0600) or die "mkfifo failed - directory write protected?";
...
#  packing != c0
   if ($fast == 0) {
      ...
      mkfifo("$out.p1",0600) or die "mkfifo failed - directory write protected?";
      mkfifo("$out.p2",0600) or die "mkfifo failed";
      mkfifo("$out.p3",0600) or die "mkfifo failed";
